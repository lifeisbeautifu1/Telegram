...
const Chat: React.FC<ChatProps> = ({ socket }) => {
  ...
  const sender = chat && getSenderFull(user!, chat?.users!);
  const chatName =
    chat && chat?.is_group_chat ? chat?.chat_name : sender?.username;
  const image_url =
    chat && chat?.is_group_chat ? chat?.image_url : sender?.image_url;
      ...
      {chat && !isChatInfo ? (
        <div className="h-full flex flex-col">
          <div className="py-1 px-4 flex items-center gap-2 border-b border-gray-200 dark:border-gray-500 dark:bg-slate-600">
            {/* @ts-ignore */}
            <Avatar letter={chatName && chatName[0]} image_url={image_url} />
            <div className="flex flex-col justify-center">
              ...
              {chat?.is_group_chat ? (
                ...
                <p className="text-xs text-gray-400">
                  last seen{' '}
                  {formatDistanceToNow(Number(sender?.last_online), {
                    addSuffix: true,
                  })}
                </p>
              ) : (
