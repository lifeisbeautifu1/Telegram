...
const SidebarSettings = () => {
    ...
    const { isEditProfile } = (0, hooks_1.useAppSelector)((state) => state.app);
    const dispatch = (0, hooks_1.useAppDispatch)();
    return (<div className="w-[30%] sidebar border-r border-gray-200 dark:border-gray-500 flex flex-col">
      <div className="py-[11px] px-4 flex  items-center gap-4 justify-center border-b border-gray-200 dark:bg-slate-600 dark:border-gray-500">
        <div className="shadow-inner flex items-center gap-2 bg-[#edeef0]/50 py-[5px] px-[8px] dark:bg-slate-500 w-full rounded-md text-xl relative ">
          ...
          <input type="text" placeholder="Search" className="bg-transparent dark:text-gray-300 dark:placeholder:text-gray-300 w-full h-full outline-none text-sm"/>
        </div>
      ...
      <div className="h-full px-2 py-4 flex flex-col dark:bg-slate-600">
        <div className={`rounded-lg p-4 flex  items-center gap-2 cursor-pointer ${isEditProfile && 'bg-sky-400 dark:bg-sky-600 text-white '}`} onClick={() => dispatch((0, app_1.setIsEditProfile)(!isEditProfile))}>
          <_1.Avatar size="lg" letter={(user === null || user === void 0 ? void 0 : user.username[0]) || 'U'} image_url={user === null || user === void 0 ? void 0 : user.image_url}/>
          <div className="flex flex-col  text-xs ">
            <h1 className="capitalize font-semibold dark:text-white">
              {user === null || user === void 0 ? void 0 : user.username}
            </h1>
            <h1 className={`${isEditProfile
            ? 'text-white'
            : 'text-gray-400 dark:text-gray-300'}`}>
              {user === null || user === void 0 ? void 0 : user.email}
            </h1>
          </div>
          <div className={`ml-auto flex items-center justify-center ${isEditProfile && 'hidden'}`}>
        ...
        <ul className="p-4 flex flex-col gap-2 h-full">
          ...
          <li className="cursor-pointer flex items-center">
            <div className="bg-rose-500 flex p-1 rounded items-center justify-center text-white">
              <bi_1.BiNotification className="w-4 h-4"/>
            </div>
          ...
          <li className="cursor-pointer flex items-center">
            <div className="bg-blue-500 flex p-1 rounded items-center justify-center text-white">
              <io_1.IoMdLock className="w-4 h-4"/>
            </div>
            <p className="text-xs ml-2 dark:text-gray-300">
          ...
          <li className="cursor-pointer flex items-center">
            <div className="bg-green-500 flex p-1 rounded items-center justify-center text-white">
              <ri_1.RiDatabase2Fill className="w-4 h-4"/>
            </div>
          ...
          <li className="cursor-pointer flex items-center">
            <div className="bg-orange-500 flex p-1 rounded items-center justify-center text-white">
              <md_1.MdOutlineComputer className="w-4 h-4"/>
            </div>
          ...
          <li className="cursor-pointer flex items-center">
            <div className="bg-sky-500 flex p-1 rounded items-center justify-center text-white">
              <io5_1.IoBrush className="w-4 h-4"/>
            </div>
          ...
          <li className="cursor-pointer flex items-center">
            <div className="bg-purple-500 flex p-1 rounded items-center justify-center text-white">
              <fa_1.FaGlobe className="w-4 h-4"/>
            </div>
          ...
          <li className="cursor-pointer flex items-center">
            <div className="bg-orange-500 flex p-1 rounded items-center justify-center text-white">
              <tb_1.TbSticker className="w-4 h-4"/>
            </div>
            <p className="text-xs ml-2 dark:text-gray-300">
              Stickers and Emoji
            </p>
      ...
      <div className="mt-auto flex items-center justify-evenly py-3 border-t border-gray-200 dark:border-gray-500 dark:bg-slate-600">
        <react_router_dom_1.Link to="/messanger/contacts" className={pathname === '/messanger/contacts' ? 'active' : ''}>
        ...
        <react_router_dom_1.Link to="/messanger/settings" className={pathname === '/messanger/settings' ? 'active' : ''}>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-7 h-7 cursor-pointer">
