...
const ChatInfo = ({ socket }) => {
    ...
    const { isEditGroupChat } = (0, hooks_1.useAppSelector)((state) => state.app);
    ...
    return (<div className="w-full h-full flex flex-col">
      <div className="w-full border-b border-gray-200 dark:border-gray-500 py-4 px-4 flex items-center justify-between text-sm dark:bg-slate-600">
        ...
        <div className="text-sky-400 cursor-pointer" onClick={() => {
            var _a, _b;
            if (isEditGroupChat) {
                dispatch((0, chat_1.renameGroupChat)());
                ...
                if (((_b = selectedChat === null || selectedChat === void 0 ? void 0 : selectedChat.group_admin) === null || _b === void 0 ? void 0 : _b.id) === (user === null || user === void 0 ? void 0 : user.id))
                    dispatch((0, app_1.setIsEditGroupChat)(true));
            }
        }}>
          {isEditGroupChat ? 'Done' : 'Edit'}
        </div>
      </div>
      {isEditGroupChat ? (<_1.EditGroupChat socket={socket}/>) : (<div className="w-full  h-full bg-slate-100 pb-12 dark:bg-slate-700 overflow-y-scroll  ">
          <div className="flex flex-col items-center w-[70%] mx-auto overflow-y-scroll">
            ...
            <h1 className="mt-4 font-medium text-sm dark:text-white">
              {selectedChat === null || selectedChat === void 0 ? void 0 : selectedChat.chat_name}
            </h1>
            <p className="text-xs text-gray-400">
              {(_a = selectedChat === null || selectedChat === void 0 ? void 0 : selectedChat.users) === null || _a === void 0 ? void 0 : _a.length} members
            </p>
            <div className="grid grid-cols-4  w-full gap-4">
              <div className="rounded-lg mt-6 bg-white dark:bg-slate-600 dark:text-sky-500 text-sky-400 flex flex-col items-center justify-center py-4  cursor-pointer" onClick={() => dispatch((0, app_1.setIsAddMembers)(true))}>
              ...
              <div className="rounded-lg mt-6 bg-white  dark:bg-slate-600 dark:text-sky-500 text-sky-400 flex flex-col items-center justify-center py-4 cursor-pointer" onClick={() => {
                dispatch((0, app_1.setAction)('LEAVE'));
            ...
            <div className="mt-6 rounded-lg bg-white  w-full flex flex-col">
              ...
              <div className="flex flex-col">
                {(_b = selectedChat === null || selectedChat === void 0 ? void 0 : selectedChat.users) === null || _b === void 0 ? void 0 : _b.map((u) => (<_1.CreateChatUser key={u.id} noOnline onChatInfo user={u}/>))}
              </div>
            </div>
          </div>
        </div>)}
